annotation_tex <- function(infile = NULL, outfile = "annotation.tex"){
    ...
    temp1 <- c("\\documentclass[a4paper,5pt,twocolumn]{article}",
                ...
                "\\usepackage{geometry}", # 
                "\\usepackage{CJK}",
                "\\geometry{left=0.8cm,right=0.2cm,top=0.8cm,bottom=0.2cm}",
    ...
    for(i in 1:nrow(herbdat000)){
        ...
        ifelse(is.na(herbdat$PROJECT), "\\vspace{4mm}", paste("\\centerline{", 
                as.character(herbdat$PROJECT), "}\\vspace{1mm}", sep = "")),
        
        #### SPECIES INFO
        ifelse(is.na(herbdat$INFRASPECIFIC_RANK),
              paste("\\large{\\textsl{\\textbf{",
                    herbdat$GENUS,"}}", ifelse(is.na(herbdat$SPECIES)|herbdat$SPECIES == "sp.", " \\textbf{ sp.}", 
                    paste(" \\textsl{\\textbf{", as.character(herbdat$SPECIES),"}}")), "\\textbf{", ifelse(is.na(herbdat$AUTHOR_OF_SPECIES),"", as.character(herbdat$AUTHOR_OF_SPECIES)), 
                    "}}", sep = ""),
              paste("\\large{\\textsl{\\textbf{",
                    herbdat$GENUS,"}} \\textsl{\\textbf{",herbdat$SPECIES,"}} \\textbf{",herbdat$AUTHOR_OF_SPECIES,"} ", 
                    herbdat$INFRASPECIFIC_RANK," \\textsl{\\textbf{",herbdat$INFRASPECIFIC_EPITHET, "}} \\textbf{", 
                    ifelse(is.na(herbdat$AUTHOR_OF_INFRASPECIFIC_RANK),"", as.character(herbdat$AUTHOR_OF_INFRASPECIFIC_RANK)) ,"}} ", sep = "")
                ),
        "\\vspace{4mm }\\\\", 
        ifelse(is.na(herbdat$DET_SOURCE), "", paste("Det. Source: ", herbdat$DET_SOURCE, "\\\\",sep = "")), 
        ##### IDENTIFICATION INFOMATION
        paste("\\rightline{",  "Det.: ", herbdat$IDENTIFIED_BY, " ", herbdat$INSTITUTION," \\hfill ", 
