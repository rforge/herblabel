bbsp_csv2herblabel  <- function(infile, outfile = NULL, 
                                ...
                                DATE_LASTMODIFIED = NA                                  )
{
    ...
    LOCAL_NAME <- rep(NA, nrow(bg_base_export_dat))
    ...
    LOCALITY  <- paste(ifelse(is.na(LOCALITY1), "", LOCALITY1),ifelse(is.na(LOCALITY2), "", LOCALITY2), sep = " ")
    REMARKS  <- paste(ifelse(is.na(REMARKS1),"", REMARKS1), 
                  ifelse(is.na(REMARKS2),"",REMARKS2), 
                  ifelse(is.na(REMARKS3), "", paste("Living Acc. No.:", REMARKS3)), sep = " ")
    res.final  <- data.frame(GLOBAL_UNIQUE_IDENTIFIER, HERBARIUM, TITLE, COLLECTOR, ADDITIONAL_COLLECTOR, 
