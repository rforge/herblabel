annotation_rtf <- function(infile = NULL,  outfile = "annotations.rtf"){
    ...
    for(i in 1:nrow(herbdat000)){
        ...
        ifelse(is.na(herbdat$PROJECT), "", paste("{\\pard\\keep\\keepn\\fi0\\li0\\brsp20\\qc\\sb400\\sa100\\fs16", 
        ...
        #####
        ifelse(is.na(herbdat$DET_SOURCE), "",  paste("{\\pard\\keep\\sb100\\sa50\\keepn\\fi0\\li0Det. Source:", as.character(herbdat$DET_SOURCE),"}") ),
        ##### IDENTIFICATION INFOMATION
            paste("{\\pard\\keep\\sb150\\sa50\\keepn\\fi0\\li0\\tqr\\tx5045 Det.: ",
                 herbdat$IDENTIFIED_BY,", ",herbdat$INSTITUTION,"  \\tab ", toupper(format(as.Date(herbdat$DATE_IDENTIFIED), 
